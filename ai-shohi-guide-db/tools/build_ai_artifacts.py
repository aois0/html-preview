#!/usr/bin/env python3
"""
Build AI-optimized artifacts for NTA consumption-tax filing guides
under 10k-token read constraints.

Current corpus (latest editions as of 2026-02-11):
  - 法人用 消費税及び地方消費税の申告書（一般用）の書き方
  - 法人用 消費税及び地方消費税の申告書（簡易課税用）の書き方
  - インボイス制度に関するお知らせ（令和7年4月）

Output layout (ai-shohi-guide-db/):
...
BASE_URL = "https://jplawdb.github.io/html-preview/ai-shohi-guide-db"
...
def write_quickstart(specs: list[SourceSpec], items_by_doc: dict[str, list[Item]]) -> None:
    lines = [
        "# AI向け 消費税申告書手引DB クイックスタート（1URL≒1万トークン制約向け）",
        "#",
        "# 基本方針:",
...
def write_llms(specs: list[SourceSpec], items_by_doc: dict[str, list[Item]]) -> None:
    lines = [
        "# AI向け 消費税申告書手引DB",
        "# Base URL: https://jplawdb.github.io/html-preview/ai-shohi-guide-db/",
        "",
        "入口（推奨）",
        "- `quickstart.txt`",
        "- `data/resolve_lite/index.json`",
...
<head>
  <meta charset=\"utf-8\" />
  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\" />
  <title>AI向け 消費税申告書手引DB</title>
  <meta name=\"description\" content=\"消費税申告書の作成手引き資料をAI向けに分割・構造化したDB\" />
...
<body>
  <h1>AI向け 消費税申告書手引DB</h1>
  <p class=\"meta\">国税庁の「消費税申告書の作成手引き」関連資料を、1URL=短い本文単位で再構成しています。</p>
  <ul>
...
Sitemap: https://jplawdb.github.io/html-preview/ai-shohi-guide-db/sitemap.xml
"""
...
def main() -> None:
    ...
    print("[done] ai-shohi-guide-db artifacts generated")
